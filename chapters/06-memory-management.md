...
+--------+-------------------+
| 遍历 regions:              |
| region_guest_base <= GPA   |
| < region_guest_base + len? |
+--------+-------------------+
         |
    +----+----+
    |         |
...
+-----------------------------------------------------------+
|                    VMM 上层调用者                           |
|          (vCPU / virtio 设备 / snapshot)                   |
+-------------------+---------------------------------------+
                    |
                    | read_obj / write_obj / get_host_address
                    v
+-------------------+---------------------------------------+
|               GuestMemoryMmap                              |
|          (实现 GuestMemory trait)                           |
|  +--------------------------------------------------+     |
|  | regions: Vec<GuestRegionMmap>                     |     |
|  |   +------------------+  +------------------+     |     |
|  |   | Region 0         |  | Region 1         |     |     |
|  |   | guest_base: 0x0  |  | guest_base: 4GiB |     |     |
|  |   | host_base: HVA_0 |  | host_base: HVA_1 |     |     |
|  |   | len: 3.25 GiB    |  | len: 剩余大小     |     |     |
|  |   +--------+---------+  +--------+---------+     |     |
|  +--------------------------------------------------+     |
+-------------------+---------------------------------------+
                    |
                    | mmap / KVM_SET_USER_MEMORY_REGION
                    v
+-------------------+---------------------------------------+
|                  Linux Kernel / KVM                        |
|  +------------------+    +-----------------------------+  |
|  | mmap 匿名映射     |    | EPT/NPT 页表               |  |
|  | (HVA -> HPA)     |    | (GPA -> HPA 硬件转换)      |  |
|  +------------------+    +-----------------------------+  |
+-----------------------------------------------------------+
```
...
为什么选择 `mmap` 匿名映射而非文件映射或 hugepage？匿名映射（`MAP_ANONYMOUS`）是最简单的方案：无需预先创建文件，无需额外的文件系统配置，且配合 Linux 的 demand paging，物理页面仅在首次访问时才真正分配。替代方案如 hugetlbfs 映射可以减少 TLB miss，在大内存场景下能提升 5-10% 的性能，但需要 Host 预先配置 hugepage 池，增加了部署复杂度。QEMU 支持多种内存后端（文件、hugepage、NVDIMM 等），但这种灵活性带来了大量的代码和潜在的安全问题。Firecracker 选择了最简单的匿名映射作为唯一方案，将 hugepage 优化留给 Host 层面透明的 THP（Transparent Huge Pages）机制处理。
...
x86_64 上的 MMIO hole（3.25 GiB - 4 GiB）是 PC 架构的历史包袱。替代方案是将所有 MMIO 设备映射到更高的地址空间（如 64 位地址区域），从而让低 4 GiB 完全可用于 RAM。但这要求 guest 内核和固件都支持高地址 MMIO 映射，增加了兼容性风险。Firecracker 选择遵循传统布局，保证了与标准 Linux 内核的兼容性。将内存分成两个 region 的处理方式虽然增加了一点代码复杂度，但这种复杂度被完全封装在 `arch` 模块内部，上层调用者无需感知。aarch64 架构没有这一历史包袱，MMIO 区域位于低地址段，RAM 从高地址开始，布局更加清晰。
