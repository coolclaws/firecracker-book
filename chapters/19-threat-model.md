...
以下图示展示了 Firecracker 信任域边界与防御层的对应关系：
...
+---------------------------+---------------------------+
                            | 边界 1: 硬件虚拟化隔离
                            | (EPT/Stage-2 页表, VT-x/VHE)
+---------------------------+---------------------------+
| VMM 域 (中等信任)                                      |
| +---------------------------------------------------+ |
| | Jailer 沙箱层                                      | |
| | namespace (PID/MNT/NET) + cgroup + chroot         | |
| | +-----------------------------------------------+ | |
| | | seccomp 层                                     | | |
| | | 白名单系统调用过滤 (每线程独立)                  | | |
| | | +-------------------------------------------+ | | |
...
+---------------------------+---------------------------+
| Guest 域 (不信任)                                      |
| Guest Kernel + 用户代码 (视为恶意)                     |
+-------------------------------------------------------+

    API 域 ----Unix Socket----> VMM
    (中等信任)   文件权限控制 + 输入校验
```

### 设计取舍

Firecracker 选择将 Host 内核视为可信基础（Trusted Computing Base）而非试图防御 Host 内核漏洞，这是一个务实但重要的边界划定。替代方案包括使用 Type-1 hypervisor（如 Xen）将 VMM 与 Host 内核完全分离，或使用 AMD SEV / Intel TDX 等机密计算技术保护 guest 免受 host 攻击。前者增加了运维复杂性和部署限制，后者在 Firecracker 诞生时尚未成熟。Firecracker 的选择基于这样的判断：在 AWS 的运维模型中，Host 内核由 AWS 自身控制和维护，其安全性可以通过严格的补丁管理和配置加固来保证。将精力集中在 VMM 层面的最小化和纵深防御上，比试图解决一个更宏大的信任问题更具工程效率。
