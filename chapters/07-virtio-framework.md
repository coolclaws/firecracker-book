...
以下是 virtqueue 的完整操作周期，展示了 Guest 驱动与 Host 设备之间的数据流动：

```
Guest 驱动                                     Host 设备 (VMM)
==========                                     ===============
...
+-----+-----------------------------+---------------------------+
      |                             |
      | EventFd 通知                | VM Exit (MMIO)
      v                             v
+-----+------------------+  +------+---------------------------+
| ioeventfd / irqfd      |  | MmioTransport                    |
| (内核态快速路径)        |  | - 设备发现 & 寄存器访问           |
|                         |  | - 状态机管理                     |
+-------------------------+  | - 特性协商                       |
                             +------+---------------------------+
              ...
              |                     |                     |
       +------+------+     +-------+-----+     +---------+---+
       | Net 设备     |     | Block 设备   |     | Vsock 设备  |
       | (virtio-net) |     | (virtio-blk) |     | (virtio-    |
       |              |     |              |     |  vsock)     |
       +------+-------+     +------+-------+     +------+-----+
              |                     |                     |
              v                     v                     v
       +------+-------+     +------+-------+     +-------+----+
       | Queue (TX/RX)|     | Queue (REQ)  |     | Queue      |
...
选择 MMIO 而非 PCI transport 的决策是 Firecracker 整体设计哲学的缩影。PCI transport 的优势在于：成熟的设备发现协议（PCI enumeration）、MSI-X 中断支持（可实现 per-queue 中断，减少中断共享带来的延迟）、以及 Guest OS 中更成熟的驱动生态。但 PCI transport 的代价也是显著的：完整的 PCI 模拟需要实现 PCI 配置空间（256 字节或 4KB）、BAR 地址分配与映射、中断路由（INTx/MSI/MSI-X）等机制，代码量至少是 MMIO transport 的 5-10 倍。在安全关键场景中，每一行代码都是潜在的攻击面。Firecracker 选择 MMIO transport 的本质是用"设备发现的便利性"和"可扩展性"换取"更小的 TCB 和更简单的审计"。另外，split virtqueue 而非 packed virtqueue 也是类似的取舍——packed virtqueue（virtio 1.1 引入）通过更紧凑的内存布局提升缓存命中率，但实现复杂度更高，且在 Firecracker 的低设备数量场景下收益有限。
