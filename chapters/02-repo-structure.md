...
下表展示了各核心 crate 之间的依赖方向与协作职责：

```
+------------------+          +------------------+
|   firecracker    | -------> |   api_server     |
|   (入口 binary)  |          | (HTTP 解析/路由)  |
+--------+---------+          +--------+---------+
         |                             |
         |     VmmAction / VmmResponse |
         |                             v
         +-------------------> +------------------+     +------------------+
                               |       vmm        | --> |    snapshot       |
                               |  (核心生命周期)   |     | (状态序列化/恢复) |
            ...
            v                  v                 v                  v
   +---------------+  +---------------+  +---------------+  +---------------+
   |  kvm-ioctls   |  |  vm-memory    |  |  vm-superio   |  | seccompiler   |
   | (KVM ioctl    |  | (Guest 内存   |  | (串口模拟)    |  | (seccomp BPF  |
   |  类型安全封装) |  |  映射与访问)  |  |               |  |  规则编译)    |
   +---------------+  +---------------+  +---------------+  +---------------+

   +------------------+
   |     jailer       |  ← 独立 binary，不依赖 vmm
   | (安全沙箱设置)   |
   +------------------+
```
...
**为什么采用 Cargo Workspace 单仓库而非多仓库？** Firecracker 将所有 crate 放在同一个仓库中通过 workspace 管理，而非像 `rust-vmm` 社区那样将每个 crate 拆分为独立仓库。原因是 Firecracker 的各 crate 之间存在紧密的版本耦合——`vmm` crate 的接口变更往往需要同步修改 `api_server` 和 `firecracker` 入口。单仓库确保所有变更在一个 PR 中原子提交，共享同一个 `Cargo.lock`，避免了多仓库方案中常见的版本不一致和"菱形依赖"问题。代价是仓库体积更大，且不适合将内部 crate 独立发布到 crates.io——但 Firecracker 本就不需要将内部模块作为独立库发布。

**为什么 jailer 作为独立 binary 而非 vmm 的子模块？** jailer 以 root 权限运行来设置 namespace、cgroup 和 seccomp，然后 `exec()` 进 Firecracker 进程。如果将 jailer 逻辑嵌入 vmm，则整个 vmm 的依赖树都会在 root 权限下被加载，违反最小权限原则。独立 binary 确保特权代码的依赖链最短——jailer 只依赖 `libc` 等极少数 crate，攻击面被控制在最小范围。
