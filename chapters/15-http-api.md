...
选择 Unix Domain Socket 而非 TCP 意味着放弃了远程管理能力——调用者必须与 Firecracker 在同一台 host 上。Firecracker 团队认为这是正确的取舍：在 AWS Lambda 的架构中，一个 host 级别的管理组件（如 containerd 或自研的 placement agent）负责在本地与 Firecracker 交互，远程控制由上层编排系统处理。如果允许 TCP 远程访问，就需要引入 TLS、认证令牌、网络 ACL 等一系列安全机制，极大增加了复杂度和攻击面。另一个被考虑过的替代方案是 gRPC，它提供了强类型的 IDL 和代码生成，但其依赖链（protobuf、HTTP/2 栈）过于庞大，与 Firecracker 轻量级的定位不符。
...
自研 micro_http 的代价是放弃了 HTTP 协议的完整性——不支持 chunked transfer encoding、不支持 HTTP pipelining、不支持 WebSocket 升级。Firecracker 团队评估后认为这些特性在 VMM 控制场景中完全不需要：每个请求都是独立的 JSON 配置操作，请求体通常不超过几 KB，不存在流式传输需求。另一个取舍是只支持 GET 和 PUT 两种方法——没有 DELETE（移除已配置的设备通过 PUT 空配置或重启实现），没有 PATCH（部分更新通过整体 PUT 替换实现）。这种极简方法集虽然牺牲了 REST 的完整语义，但大幅简化了路由和处理逻辑，减少了边界情况。
...
+------------------+
      |
      | VmmAction + JSON 反序列化后的配置结构体
      v
+------------------+     channel (tx)     +------------------+
|  API Server 线程 | ------------------> |  VMM 主线程       |
|                  |                     |  处理 VmmAction   |
|                  | <------------------ |  修改 VmResources |
+------------------+     channel (rx)     |  或执行 VM 操作   |
      |                                  +------------------+
      | VmmActionResult
      v
  构造 HTTP Response（200 / 400 / 500）
      |
      v
  返回客户端
```
...
+------------------------------------------------------+
         |                |                  |
         v                v                  v
   Unix Socket      VmmAction 枚举     强类型配置结构体
   读写              定义请求类型       MachineConfig,
                                       BootSourceConfig...
                          |
                          v
                  +---------------+
