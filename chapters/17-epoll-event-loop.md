...
```
event-manager crate             VMM 内部
+-------------------------+     +-------------------------------+
| EventManager            |     | EpollContext                  |
|  - epoll_fd             |     |  - 注册 API socket fd         |
|  - fd_to_subscriber map |<----+  - 注册 vCPU eventfd          |
|  - run_with_timeout()   |     |  - 注册 virtio eventfd        |
+--------+----------------+     |  - 注册 signalfd              |
         |                      +-------------------------------+
         | 分发事件
         v
...
单线程事件驱动架构是 Firecracker 最重要的架构决策之一。备选方案包括多线程 reactor 模型（如 Tokio 的工作窃取线程池）和 per-device 独立线程模型。多线程 reactor 能提供更高的事件吞吐量，但引入了锁竞争、线程同步和非确定性执行顺序等复杂性，这对安全审计是灾难性的——并发 bug 往往只在特定时序下触发，极难通过测试覆盖。per-device 线程模型虽然概念上更简单，但线程数量随设备数线性增长，增加了上下文切换开销和内存占用。Firecracker 的单线程模型之所以可行，根本原因在于每个 microVM 的事件规模有限（几十个 fd），且真正的计算并行性已委托给独立的 vCPU 线程——事件循环只处理控制平面，不参与数据平面的密集计算。这种控制平面与数据平面的分离，是兼顾简洁性与性能的关键。
