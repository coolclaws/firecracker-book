...
VmResources 采用集中式聚合而非分布式配置管理。替代方案是让每个子系统（CPU、内存、设备）各自持有和验证自己的配置，通过依赖注入在构建时组装。分布式方案的好处是模块化更彻底，每个子系统可以独立演化；但其缺点是跨组件的一致性约束难以表达——例如 "vCPU 数量必须与中断控制器的引脚映射兼容" 这类约束需要一个全局视角。Firecracker 选择集中式设计，是因为 microVM 的配置参数总量不大（十几个字段），集中管理的复杂度完全可控，而获得的全局一致性检查能力对正确性至关重要。此外，集中式设计使得配置文件方式和 API 方式可以共享同一份验证逻辑，避免了规则分散导致的不一致风险。
...
配置验证链
  ...
  用户输入（API 请求 / 配置文件 JSON）
      |
      +---> HTTP 层验证
      |       +---> 请求格式检查（Content-Type, body size）
      |
      +---> JSON 反序列化验证
      |       +---> serde 强类型解析
      |       +---> 字段类型检查、必填字段检查
      |       +---> 拒绝未知字段
      |
      +---> 单项配置验证（写入 VmResources 时）
      |       +---> MachineConfig: vcpu_count 范围, mem_size 对齐
      |       +---> BootSource: 内核文件存在性, boot_args 长度
      |       +---> BlockDevice: drive_id 唯一性, 文件存在性
      |       +---> NetworkInterface: iface_id 唯一性, MAC 格式
      |
      +---> 配置存入 VmResources（single source of truth）
      |
      |  ======== InstanceStart 触发 ========
      |
      +---> 预启动全局验证
      |       +---> BootSource 是否已设置（必需项）
      |       +---> 设备 ID 无冲突
      |       +---> 整体配置一致性
      |
      +---> build_microvm() 运行时验证
...
两阶段验证是 "尽早报错" 和 "TOCTOU（Time-of-check to time-of-use）现实" 之间的折中。一种极端是把所有验证推迟到 `build_microvm()` 阶段——实现简单，但用户体验差，因为简单的拼写错误要等到启动时才暴露。另一种极端是在配置时就尝试打开文件句柄、探测 KVM 能力，做到完全的早期验证——但这会引入资源持有问题（配置阶段就占住文件锁或设备），也无法避免配置后到启动前这段时间窗口内的环境变化。Firecracker 的两阶段方案在实践中覆盖了绝大多数用户错误（配置时），同时对不可避免的环境竞态保持诚实（构建时），是工程实用主义的体现。
