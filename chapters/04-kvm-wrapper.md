...
通过 KVM ioctl 创建并运行虚拟机的完整调用路径：

```
用户态 Firecracker 进程                        内核态 KVM 模块
========================                      ========================

open("/dev/kvm")
  +---> fd = kvm_fd                           分配 KVM 全局上下文
        |
        | ioctl(kvm_fd, KVM_CHECK_EXTENSION)
        +---> 检查 Cap::Irqchip,             返回 capability 支持情况
        |     Cap::Ioeventfd, ...
        |
        | ioctl(kvm_fd, KVM_CREATE_VM)
        +---> vm_fd                           分配 struct kvm (EPT 根页表)
              |
              | ioctl(vm_fd, KVM_SET_USER_MEMORY_REGION)
              +---> 注册 guest 内存            填充 EPT 映射
              |
              | ioctl(vm_fd, KVM_CREATE_IRQCHIP)
              +---> 创建中断控制器             内核态 LAPIC + IOAPIC
              |
              | ioctl(vm_fd, KVM_CREATE_VCPU, index)
              +---> vcpu_fd                   分配 struct kvm_vcpu
                    |
                    ...
                    +---> 设置 CPUID 表         配置 guest 可见的 CPU 特性
                    |
                    | ioctl(vcpu_fd, KVM_RUN)   ──── VMENTRY ────>
                    +---> 阻塞等待 VmExit       Guest 代码执行
                    |     <──── VMEXIT ────
                    |
                    +---> 处理 VmExit 原因
                          +---> MMIO           分发给 VMM 设备模拟
...
**为什么采用三层 ioctl 而非扁平化 API？** KVM 的 system / VM / vCPU 三层 ioctl 设计并非偶然——它精确反映了虚拟化资源的所有权层次。系统级操作（如查询 capability）是全局的、无状态的；VM 级操作（如内存注册）影响整个虚拟机的所有 vCPU；vCPU 级操作（如寄存器设置）只影响单个 vCPU。`kvm-ioctls` 用三个 Rust 类型（`Kvm`、`VmFd`、`VcpuFd`）精确映射这三层，利用 Rust 类型系统在编译期阻止"在错误的层级调用 ioctl"的错误——例如你无法在 `Kvm` 上调用 `set_regs()`，因为该方法只存在于 `VcpuFd` 上。扁平化 API（如一个通用的 `kvm_ioctl(fd, cmd, arg)` 函数）则会丢失这种静态保证。
...
**为什么复用 `rust-vmm/kvm-ioctls` 而非自行封装？** 自行封装可以完全控制 API 设计和演进节奏，但 KVM ioctl 封装是一个"高投入、低差异化"的工程——每个 VMM 项目需要的封装逻辑高度一致，而正确处理所有边界条件（如 `EINTR` 重试、跨架构差异、内核版本兼容性）需要大量实战积累。`kvm-ioctls` 由 Firecracker、Cloud-Hypervisor、CrosVM 等多个项目共同使用和验证，bug 修复和安全补丁被多个项目共享。代价是 Firecracker 需要跟随上游的 API 变更节奏，偶尔会因上游接口调整而需要适配。但与维护一套私有封装的长期成本相比，这个代价微不足道。

**为什么 `VcpuFd` 可以独立于 `VmFd` 的生命周期存在？** 在底层实现中，`KVM_CREATE_VCPU` 返回的文件描述符是通过 `anon_inode_getfd` 创建的独立 fd，内核通过引用计数管理 VM 资源——只要存在任何关联的 fd（VM fd 或 vCPU fd），VM 的内核数据结构就不会被释放。`kvm-ioctls` 利用这一语义，让 `VcpuFd` 不持有对 `VmFd` 的引用，使其可以被 `move` 到独立的 vCPU 线程中而无需 `Arc` 包装。这正是 Firecracker 能够以零开销将每个 vCPU 分配到独立线程的基础。
