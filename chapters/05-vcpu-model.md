...
为什么采用 thread-per-vCPU 模型，而不是基于 async 运行时的协作式调度？核心原因有三：第一，`KVM_RUN` 是一个阻塞式系统调用，将其放进 async executor 会占用线程池资源，抵消异步的优势；第二，thread-per-vCPU 模型让每个 vCPU 线程的栈和状态完全独立，便于 OS 调度器进行 CPU 亲和性优化；第三，Firecracker 的目标场景通常只有 1-2 个 vCPU，线程数量极少，线程管理开销可以忽略。替代方案如 "多 vCPU 复用单线程 + epoll" 会引入复杂的状态机逻辑，增加代码复杂度和出错概率，与 Firecracker 追求简单的哲学背道而驰。
...
以下是 vCPU run loop 的完整执行流程，涵盖从进入 guest 模式到退出处理再到重新进入的循环：
...
CPUID 过滤采用的是"白名单"策略而非"黑名单"策略。黑名单策略（仅屏蔽已知危险特性）更宽松，能让 guest 利用更多硬件能力，但每当 CPU 厂商引入新指令集扩展时，都可能因遗漏而产生安全隐患。白名单策略（仅放行已知安全的特性）则相反——默认拒绝一切，只开放经过验证的特性。这在安全关键场景下是更稳健的选择，代价是可能无法第一时间利用最新的 CPU 特性。Firecracker 还支持自定义 CPU 模板（CPU template），允许运维人员在可迁移性和性能之间自行选择平衡点：使用更严格的模板提高跨主机迁移的成功率，或使用更宽松的模板发挥特定硬件的性能优势。
